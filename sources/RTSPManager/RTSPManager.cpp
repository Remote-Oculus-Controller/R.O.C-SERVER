...
int RTSPManager::createServer(int id, int port)
{
	std::unique_lock<std::mutex> lock(_lock);
	this->_done = false;

	std::thread * thread = new std::thread(&RTSPManager::createRTSPServer , this , id , port , this->watcher);
	thread->detach();
